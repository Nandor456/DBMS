...
function TableList({updateTrigger}) {
  ...
  const handleDelete = async (tableName) => {
    try {
        const response = await fetch("http://localhost:4000/database/table/delete", {
            method: "DELETE",
        ...
        setTablesByDb((prevTables) => ({
            ...prevTables,
            [activeDatabase]: prevTables[activeDatabase].filter((item) => item !== tableName),
        }));
        } catch (error) {
      ...
      {tables.length > 0 ? (
        <ul>
          {tables.map((table, index) => (
            <li key={index}>
                <label>{table}</label>
                <button onClick={() => handleDelete(table)} style={{ backgroundColor: "red", color: "white" }}>
                    Delete
                </button>
            </li>
          ))}
